...
| `/гем мышление <задача>` | То же, но сильной цепочкой, минуя настройку раздела. Синоним — `/gem think` |
| `/ask <вопрос>` | Вопрос нейросети, выбранной в `/ai` (с учётом истории диалога) |
| `/draw <описание>` | Сгенерировать картинку через OpenRouter (**платно**) |
...
│   │   ├── ai.ts              # /гем (+ «мышление») /ask /draw /ai /reset + картинки на вход
...
Порядок цен: `openai/gpt-image-1-mini` в разрешении 1024×1024 и качестве `low`
обходится примерно в полцента за картинку — около тысячи изображений на пять
долларов. Точную стоимость каждого вызова OpenRouter возвращает сам, и бот
показывает её в подписи под картинкой.

Блок можно оставить пустым — тогда отключится только `/draw`, остальное работает.
...
GEMINI_API_KEY=AIza...               # чтобы работали /гем, /ask и разбор картинок
OPENROUTER_API_KEY=sk-or-...         # чтобы работал /draw (платно)
```
...
| `GEMINI_CHAIN_THINK` | см. `.env.example` | Цепочка для `/гем мышление` — дневная норма невелика |
...
| `OPENROUTER_API_KEY` | — | Ключ OpenRouter — нужен только для `/draw` |
| `OPENROUTER_BASE_URL` | `https://openrouter.ai/api/v1` | Базовый URL OpenRouter |
| `OPENROUTER_IMAGE_MODEL` | `openai/gpt-image-1-mini` | Модель генерации картинок |
| `OPENROUTER_IMAGE_RESOLUTION` | `1024x1024` | Размер картинки |
| `OPENROUTER_IMAGE_QUALITY` | `low` | Качество: влияет на цену сильнее всего |
| `OPENROUTER_IMAGE_FORMAT` | `png` | Формат вывода |
...
**`/draw`: «На балансе OpenRouter закончились деньги» (402)**
...
**`/draw` отвечает «нейросеть не подключена»**
