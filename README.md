...
Порядок цен (замерено на `openai/gpt-image-1-mini`, качество `low`): квадратная
картинка 1024×1024 стоит **$0,0022** — около 450 изображений на доллар.
Точную стоимость каждого вызова OpenRouter возвращает сам, и бот показывает
её в подписи под картинкой.

Ценой управляет **пропорция**, а не разрешение. Модель отдаёт картинку одной
из трёх форм, и от формы зависит число оплачиваемых токенов: квадрат — 272,
портрет и ландшафт — 400–408. Если не задать `aspect_ratio` явно, модель
выбирает форму сама и цена гуляет в полтора раза. Параметр `resolution`
(`512`/`1K`/`2K`/`4K`) эта модель игнорирует вовсе — на «512» и на «1K»
приходит одно и то же изображение.
...
| `OPENROUTER_IMAGE_ASPECT_RATIO` | `1:1` | Пропорция. Именно она определяет цену: квадрат $0,0022, портрет/ландшафт $0,0033 |
| `OPENROUTER_IMAGE_RESOLUTION` | — | Ступень `512`/`1K`/`2K`/`4K`. Основная модель игнорирует, пусто — не отправляется |
| `OPENROUTER_IMAGE_QUALITY` | `low` | Качество |
| `OPENROUTER_IMAGE_FORMAT` | `jpeg` | Формат вывода. На цену не влияет, на вес файла — в 30 раз |
