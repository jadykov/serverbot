...
export interface TextGenerationOptions {
  ...
  /**
   * Разрешить модели сходить в интернет за свежими данными.
   *
   * Понимает это пока только Gemini: у него поиск встроен как инструмент
   * (grounding with Google Search), запрос уходит с тем же ключом, и модель
   * сама решает, нужен ли поиск для этого вопроса. Остальные провайдеры флаг
   * молча игнорируют — своего поиска у них нет, а падать из-за незнакомой
   * опции им незачем.
   *
   * Ссылки на источники провайдер дописывает в конец ответа: без них живой
   * поиск бесполезен — проверить, откуда взялась цифра, было бы нечем.
   */
  webSearch?: boolean;
}
