...
export const config = {
  ...
  gemini: {
    ...
    chains: {
       ...
       *
       * Цепочка отдельная по той же причине, что и голосовая, — из-за Gemma.
       * Поиск в интернете у Gemini устроен как инструмент (tools), а Gemma
       * инструментов не понимает вовсе и отвечает 400. Стоит же она в основной
       * цепочке первой, так что запрос с поиском разбился бы о первый шаг,
       * причём неперебираемо: это не «кончилась норма», а «модель так не
       * умеет». Поэтому здесь только Gemini, в том же порядке, что и всюду:
       * сначала lite с их щедрой дневной нормой, флеши — хвостом.
       *
       * Норма у самого поиска считается отдельно от обычных запросов:
       * 5000 запросов с поиском в месяц на бесплатном тарифе. На нашу группу
       * это с многократным запасом — потому поиск и включается словом, а не
       * идёт по каждому вопросу.
       */
