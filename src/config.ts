...
export const config = {
  ...
  gemini: {
    ...
    chains: {
       ...
       * по убыванию силы, от 3.7 до 3-preview, а ниже — та же логика запаса,
       * что в основной: сперва Gemma с её огромной дневной нормой, и только
       * под ней прошлое поколение по 20 запросов на модель.
       *
       * Про Gemma здесь нужна оговорка, и неприятная. Этой же цепочкой идут
       * «!файл» и присланные книги: окном в 262 тысячи токенов Gemma ещё
       * вытянет (предел файла — 150 тысяч, см. config.docs.maxTokens), а вот
       * PDF не принимает вовсе. Отказ будет 400 — то есть 'bad-request',
       * а его цепочка не перебирает (см. RETRYABLE в chain.ts). Значит,
       * дойди сюда запрос с PDF, человек увидит ошибку, хотя 2.5 под ней
       * ответили бы. Случай редкий — до Gemma доходит только когда все четыре
       * флеша за сутки выдохлись, — но если он станет обычным, Gemma из этой
       * цепочки надо убирать, а не двигать: место ниже её не спасает.
       */
