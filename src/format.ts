 ...
 *
 * Поддерживается: жирный, курсив, зачёркнутый, подчёркнутый (++текст++),
 * инлайн-код, блоки кода
 * с подсветкой языка, ссылки, цитаты, списки (дефис превращается в «•»),
...
export function markdownToTelegramHtml(markdown: string): string {
  ...
  text = text
    ...
    //
    // Отступ перед решётками не ограничен тремя пробелами, как того требует
    // CommonMark для настоящего заголовка: здесь не разбор чужого маркдауна
    // по стандарту, а разбор того, что пишет нейросеть, — а она иногда
    // оформляет раздел решётками с отступом в четыре и больше пробелов
    // (визуальная привычка, не код). По CommonMark это перестало бы быть
    // заголовком и осталось бы «###» на экране — что и происходило.
    .replace(/^[^\S\n]*#{1,6}[^\S\n]+(.+?)[^\S\n]*#*$/gm, '<b>$1</b>')
    // Горизонтальная линия. Тот же отступ без ограничения — по той же причине.
    .replace(/^[^\S\n]*(?:-[^\S\n]*-[^\S\n]*-|\*[^\S\n]*\*[^\S\n]*\*|_[^\S\n]*_[^\S\n]*_)[-*_ \t]*$/gm, '––––––––––')
    // Маркеры списка: - * + -> • (единый маркер со всей вёрсткой бота,
    // новостей и источников; нумерованные списки оставляем как есть).
    // Ведущую · тоже подхватываем: модель по привычке иногда ставит её сама.
    .replace(/^([^\S\n]*)[-*+·][^\S\n]+/gm, '$1• ')
    // Цитаты (символ > уже экранирован в &gt;). Тот же отступ без ограничения.
  ...
  text = text
    ...
    // Подчёркнутый: ++текст++ (для ++ нужен свой синтаксис — в Markdown
    // подчёркивания нет, а __ уже занято под жирный).
    .replace(/\+\+(?=\S)([^+\n]*\S)\+\+/g, '<u>$1</u>')
