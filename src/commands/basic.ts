 ...
 *
 * Список «Главное» — целиком в <pre>: моноширинный блок, где у каждой строки
 * дефис-разделитель стоит на одной и той же вертикали (см. alignCommandList).
 * Раньше команда была отдельным <code>-куском ради копирования одним тапом
 * без аргумента-подсказки рядом — от этого пришлось отказаться ради ровной
 * колонки: копировать теперь придётся строку целиком и стирать лишнее самому.
 * Выровнять текст вручную пробелами нельзя — вне <pre> шрифт не моноширинный,
 * и одинаковое число пробелов на разных строках даёт разную ширину.
 */

/** Строки списка «Главное»: команда с аргументом-подсказкой и её описание. */
const MAIN_COMMANDS: ReadonlyArray<readonly [command: string, description: string]> = [
  ['гем вопрос', 'спросить'],
  ['гем !контекст задача', 'модель посильнее, разбор подробный'],
  ['гем !сеть вопрос', `поиск в интернете (${config.webQuota.perUserPerDay} в день)`],
  ['гем !размышление вопрос', `долгое размышление (${config.deepQuota.perUserPerDay} в день)`],
  ['гем !файл что сделать', 'ответ файлом: md, txt, html и другие'],
  ['гем !нарисуй описание', `картинка (${config.imageQuota.perUserPerDay} в день)`],
  ['гем !трек описание', `песня с вокалом (${config.trackQuota.perUserPerDay} в день)`],
  ['гем !скажи текст', 'озвучить, голос и манеру можно попросить своими словами*'],
  ['гем !расшифруй', 'реплаем на голосовое — выпишу сказанное дословно'],
  ['гем !расшифруй с ответом', 'реплаем — отвечу по существу, без выписки'],
];

/**
 * Дописывает пробелы до самой длинной команды — так дефис после нужного
 * столбца выравнивается по всем строкам сразу, без ручного подбора отступов.
 */
function alignCommandList(rows: ReadonlyArray<readonly [string, string]>): string {
...
const HELP_TEXT = [
  ...
  '',
  '<b>Главное</b>',
  `<pre>${escapeHtml(alignCommandList(MAIN_COMMANDS))}</pre>`,
  '',
  ...
  '',
  '<b>Голос от меня.*</b> Просто попросите словами: голос (пол, тембр — низкий,',
  'высокий, бодрый, твёрдый, мягкий, яркий, уверенный) и манеру — как угодно:',
  '<i>«скажи низким, испуганным шёпотом: они идут»</i>. Без текста прочитаю то,',
  'на что вы ответили, иначе — свой последний ответ.',
  '',
  '<b>Голос мне.</b> В личке просто наговорите — отвечу по смыслу. В группе реплаем',
  'с <code>!расшифруй</code> — дословная выписка, а <code>!расшифруй с ответом</code>',
  'или сразу вопрос по записи — отвечу по сказанному, без выписки.',
  '',
  `<b>Песни.</b> Стиль и слова соберу сам, слова покажу сразу. Реплаем на сообщение —`,
  `песня будет про него. Длительность словами («полминуты»), по умолчанию и не больше`,
