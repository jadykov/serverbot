 ...
 *
 * Список «Главное» — не таблица, а строки живой команды. Команду читают,
 * чтобы её потом набрать руками, а не скопировать тапом, — колонка с дефилем
...
const MAIN_COMMANDS: ReadonlyArray<{ command: string; arg?: string; description: string }> = [
  ...
  { command: 'гем !сеть', arg: 'вопрос', description: `поиск в интернете (${config.webQuota.perUserPerDay} в день)` },
  {
    ...
    arg: 'вопрос',
    description: `долгое размышление (${config.deepQuota.perUserPerDay} в день)`,
  },
  { command: 'гем !файл', arg: 'формат заказ', description: 'ответ файлом: md, txt, html и другие' },
  { command: 'гем !нарисуй', arg: 'описание', description: `картинка (${config.imageQuota.perUserPerDay} в день)` },
  {
    command: 'гем !трек',
    arg: 'описание',
    description: `песня с вокалом (${config.trackQuota.perUserPerDay} в день)`,
  },
  { command: 'гем !скажи', arg: 'текст', description: 'озвучить, голос и манеру можно попросить своими словами*' },
  { command: 'гем !расшифруй', description: 'реплаем на голосовое — выпишу сказанное дословно' },
  { command: 'гем !ответь', description: 'реплаем на голосовое — отвечу по существу, без выписки' },
];

/** Строит пары строк для одной команды: «команда [плейсхолдер]» и отступ-описание. */
...
const HELP_TEXT = [
  ...
  'и отвечу по нему, а без вопроса перескажу. Длинные файлы читаю целиком.',
  '',
  ...
  'коротко, на сложное — подробно, но всегда одним сообщением Telegram',
  '(не более 3500 знаков — это потолок 4096 минус ~15% запаса).',
  'Файлом приезжают только явный <code>!файл</code> и <code>!размышление</code> —',
  'обычные ответы файлом не уезжают никогда.',
  '',
  ...
  'с <code>!расшифруй</code> — дословная выписка, а с <code>!ответь</code>',
  ...
  '',
  '<b>Про платное.</b> <code>!сеть</code> ищет за бесплатные кредиты Tavily,',
  'а <code>!размышление</code> думает над одним вопросом минуту-другую — без истории раздела,',
  'но со свежими страницами. Разбор найденного и размышление пишет платная модель,',
  'отсюда и дневные нормы.</blockquote>',
  '',
