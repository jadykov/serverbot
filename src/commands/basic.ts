...
const HELP_TEXT = [
  ...
  '<code>/гем !контекст задача</code> — модель посильнее, разбор подробный',
  `<code>/гем !нарисуй описание</code> — картинка (платно, ${config.imageQuota.perUserPerDay} в день)`,
  '<code>/гем !скажи «мужской» текст</code> — озвучить; в кавычках — голос и манера',
  `<code>/гем !трек описание</code> — песня с вокалом (платно, ${config.trackQuota.perUserPerDay} в день)`,
  '<code>/гем !найди что искать</code> — поиск по переписке раздела, по смыслу',
  `<code>/гем !сеть вопрос</code> — поиск в интернете (платно, ${config.webQuota.perUserPerDay} в день)`,
  `<code>/гем !размышление вопрос</code> — долгое размышление (платно, ${config.deepQuota.perUserPerDay} в день)`,
  '<code>/гем !файл что сделать</code> — ответ файлом: md, txt, html и другие',
  ...
  '<code>/гем !личка</code> реплаем — пришлю сообщение вам в личные',
  '',
  ...
  '',
  '<b>Картинки.</b> В группе нужна подпись к снимку: <code>/гем что здесь написано</code>.',
  'Подпись <code>/гем !где здесь клапан</code> — обведу найденное рамкой на фото.',
  ...
  '',
  '<b>Файлы от меня.</b> Формат — первым словом: <code>!файл html график продаж</code>.',
  ...
  '',
  '<b>Поиск и размышление.</b> <code>!найди</code> ищет по нашей переписке и бесплатен,',
  '<code>!сеть</code> ходит в интернет и даёт ссылки на источники, а <code>!размышление</code> думает',
  'над одним вопросом минуту-другую — без истории раздела, но со свежими страницами.',
  'Две последние платные, отсюда и нормы.</blockquote>',
  '',
