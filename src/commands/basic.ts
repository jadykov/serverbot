...
const HELP_TEXT = [
  ...
  '',
  '<b>Нейросети</b>',
  '<i>Слова-команды пишутся с восклицательным знаком — иначе бот примет их</i>',
  '<i>за обычный вопрос. «/гем скажи, что такое рекурсия» — это вопрос,</i>',
  '<i>«/гем !скажи что такое рекурсия» — озвучка.</i>',
  '',
  '/гем <i>запрос</i> — спросить Gemini (то же самое: /gem)',
  '/гем !контекст <i>задача</i> — то же, но моделью посильнее (то же: /gem !context)',
  `/гем !нарисуй <i>описание</i> — картинка вместо текста (то же: /gem !draw).` +
    ` Сначала уточню замысел парой вопросов: это единственное платное место,` +
    ` поэтому не больше ${config.imageQuota.perUserPerDay} картинок в день на человека`,
  '/гем !скажи <i>текст</i> — озвучить голосом (то же: /gem !say). Без текста прочитаю сообщение,',
  '<i>на которое вы ответили, а если реплая нет — свой последний ответ</i>',
  '/гем !найди <i>что искать</i> — поиск по переписке раздела, по смыслу (то же: /gem !find)',
  ...
  '<i>А если подпись начинается с «!где» — обведу найденное рамкой:</i>',
  '<i>/гем !где здесь клапан</i>',
  '<i>Это же работает реплаем на уже отправленный снимок — и рамки,</i>',
  '<i>и обычные вопросы к нему.</i>',
  '',
  ...
  '<i>Но на ответ реплаем моему сообщению я отвечу и без команды: это же</i>',
  '<i>продолжение разговора, а не новый вопрос.</i>',
