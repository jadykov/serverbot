 ...
 *
 * Главное здесь — что видно сразу, а что нет. Наверху остаются нейросети
 * и правило про восклицательный знак: за ними приходят каждый день. Всё
 * остальное — работа с картинками и файлами, проверочные команды, служебное —
 * убрано под спойлер: понадобится это раз в месяц, а место в сообщении
 * занимало столько же, сколько нужное.
 *
 * <tg-spoiler> Telegram показывает замазанным текстом, который открывается
 * по нажатию. Внутри допустимо форматирование и переносы строк, но не блочные
 * элементы — поэтому внутри только <b>, <i> и <code>.
 */
const HELP_TEXT = [
  '<b>Нейросети</b>',
  '',
  `/гем <i>запрос</i> — спросить Gemini. Латиницей: /gem, слова-переключатели те же`,
  '/гем !контекст <i>задача</i> — моделью посильнее, ответ одним сообщением',
  '/гем !контекст <i>md|txt|html</i> <i>задача</i> — тот же ответ файлом, без ограничения длины',
  `/гем !нарисуй <i>описание</i> — картинка. Уточню замысел вопросами; ` +
    `это единственное платное место, ${config.imageQuota.perUserPerDay} в день на человека`,
  '/гем !скажи <i>текст</i> — озвучить голосом. Голос кавычками: !скажи «низкий» …',
  '/гем !найди <i>что искать</i> — поиск по переписке раздела, по смыслу',
  '/гем !файл <i>что сделать</i> — пришлю отдельным файлом, а не сообщением',
  '/гем !личка <i>(реплаем)</i> — перешлю это сообщение вам в личные сообщения',
  '',
  '/режим — цепочка моделей и свой промпт для раздела',
  '/reset — очистить историю диалога',
  '',
  '<b>❗ Слова-команды пишутся с восклицательным знаком.</b>',
  '<i>Иначе я приму их за обычный вопрос:</i>',
  '<code>/гем скажи, что такое рекурсия</code> <i>— вопрос,</i>',
  '<code>/гем !скажи что такое рекурсия</code> <i>— озвучка.</i>',
  '',
  '<b>Подробности</b> <i>(нажмите, чтобы раскрыть)</i>',
  '<tg-spoiler>',
  '<b>Картинки и файлы</b>',
  'Картинку можно просто прислать — я её разберу. В группе добавьте подпись',
  'с командой: <code>/гем что здесь написано</code>.',
  'Подпись с «!где» — обведу найденное рамкой: <code>/гем !где здесь клапан</code>.',
  'Всё это работает и реплаем на уже отправленный снимок.',
  '',
  'Файл — тоже присылайте: .txt, .fb2, .pdf, .md, .csv, .json, .xml, .srt.',
  'Прочитаю и отвечу по нему; без вопроса — перескажу. Предел ~230 страниц',
  'за раз, дальше упирается минутная норма бесплатного тарифа.',
  '',
  'Без текста <b>!скажи</b> читает то сообщение, на которое вы ответили,',
  'а если реплая нет — мой последний ответ.',
  '',
  '<b>Файлы от меня</b>',
  '<code>/гем !файл смета на ремонт кухни</code> — пришлю .md',
  '<code>/гем !файл html страница с графиком продаж</code> — HTML со схемой',
  'внутри, открывается без интернета. Так же: txt, svg, csv, json, py, sql.',
  'А <code>/гем !файл</code> реплаем выгрузит в файл то сообщение, на которое',
  'вы ответили — без обращения к нейросети.',
  '',
  '<b>Проверка работоспособности</b>',
  '/ping — задержка до Telegram',
  '/marco и /polo — отвечу «Polo!» и «Marco!»',
  '/echo <i>текст</i> — повторю слово в слово',
  '/test — связь и готовность провайдеров',
  '/test ai — то же плюс живой запрос к нейросети',
  '',
  '<b>Служебное</b>',
  ...
  '</tg-spoiler>',
  '',
  '<i>Обычные сообщения запросом не считаются — обращайтесь командой. Но на ответ</i>',
  '<i>реплаем моему сообщению отвечу и без неё: это продолжение разговора.</i>',
].join('\n');
