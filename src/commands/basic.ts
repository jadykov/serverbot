...
const HELP_TEXT = [
  ...
  '<code>/гем !контекст задача</code> — модель посильнее и разбор подробный',
  `<code>/гем !нарисуй описание</code> — картинка (платно, ${config.imageQuota.perUserPerDay} в день на человека)`,
  '<code>/гем !скажи «мужской» текст</code> — озвучить; в кавычках — пол, тембр, манера',
  ...
  `<code>/гем !размысли вопрос</code> — долго подумать над одним вопросом (платно, ${config.deepQuota.perUserPerDay} в день)`,
  '<code>/гем !файл что сделать</code> — прислать ответ файлом: md, txt, html и другие',
  ...
  'Формат — первым словом: md, txt, html; есть и svg, csv, json, py, js, ts, sh, sql.',
  'Без него будет md. А <code>/гем !файл</code> реплаем выгрузит',
  'в файл то сообщение, на которое вы ответили, — без обращения к нейросети.',
  '',
  '<b>Длина ответа.</b> Обычный <code>/гем</code> отвечает по надобности:',
  'на простой вопрос — коротко, на сложный — подробно, но в одно сообщение.',
  '<code>/гем !контекст</code> разворачивается всегда, и если ответ в сообщение',
  'не влез — пришлю его файлом целиком, .md. Так со всяким длинным ответом.',
  'Формат можно назвать сразу: <code>!контекст txt</code>, <code>!контекст html</code>.',
  '',
  '<b>Голосовые вам от меня.</b> <code>/гем !скажи «низкий» смена закончена</code>.',
  'В кавычках перед текстом заказывают голос — кто говорит и как:',
  '• <b>пол</b> — мужской, женский;',
  '• <b>тембр</b> — низкий, высокий, бодрый, твёрдый, мягкий, яркий, напористый, уверенный;',
  '• <b>манера</b> — что угодно словами: <code>«шёпотом»</code>, <code>«устало»</code>, <code>«как в Warcraft 3»</code>.',
  'Голос — одно слово из первых двух списков, дальше манера словами, через запятую:',
  '<code>«мужской, устало»</code>, <code>«высокий, торжественно»</code>, <code>«женский, как диктор новостей»</code>.',
  ...
  'я по умолчанию не печатаю: её и так все слышали, — отвечаю по сказанному.',
  'Нужен именно текст записи — <code>/гем !расшифруй в текст</code>: выпишу дословно',
  ...
  'поэтому норма небольшая.',
  '',
  '<b>Долгое размышление.</b> <code>/гем !размысли что значит «понимать» текст</code> —',
  'отдам вопрос модели, которая думает перед ответом минуту-другую, и пришлю',
  'развёрнутое рассуждение (длинное приедет файлом). Историю раздела при этом',
  'не передаю: вопрос уходит голым, чтобы модель отвечала ему, а не разговору.',
  'Не путать с <code>!контекст</code>: тот бесплатный и разбирает задачу внутри',
  'беседы, а этот платный и работает с чистого листа.</blockquote>',
  '',
