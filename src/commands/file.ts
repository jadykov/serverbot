...
/**
 * Наглядная HTML-выжимка развёрнутого «!размышления» — второй файл рядом с .md.
 *
 * Разбор (.md) — это полный текст исследования, а выжимка — красочный конспект
 * главного для чтения с телефона: ключевые выводы, числа и факты, таблицы
 * и простые inline-SVG графики/схемы там, где они просятся, и чистый дизайн
 * с заголовками там, где просится он. Пишет её модель отдельным вызовом
 * без размышления (думать уже не о чем — исследование готово), поэтому
 * выжимка идёт умными уровнями, как «!файл html»: потолок FILE_MAX_OUTPUT_TOKENS
 * (16 000 по умолчанию — страница с таблицами и SVG на русском весит 8–12 тысяч).
 *
 * Отдельной нормы у выжимки нет: она — часть того же ответа, слот deepQuota
 * уже забран под размышление.
 */
const DIGEST_HTML_HINT = [
  'Ты готовишь наглядную HTML-выжимку готового исследования — красочный конспект главного для чтения с телефона.',
  'Это выжимка, а не копия: только ключевые выводы, числа и факты, без воды и без пересказа всего исследования.',
  'Формат: один самодостаточный HTML-файл: стили внутри <style>, картинки и схемы — inline SVG. ' +
    'Никаких ссылок на внешние библиотеки, шрифты и картинки: файл должен открываться без интернета.',
  'Читают с телефона: шрифт крупноват, колонка по центру, поля по краям, тёмная тема через prefers-color-scheme, ' +
    'вёрстка не разъезжается на узком экране.',
  'Данные — таблицей с рамками и подсветкой чётных строк, а не абзацем; динамику и доли — простым inline-SVG ' +
    'графиком с подписями; устройство или процесс — схематичным inline-SVG рисунком. Где графики не нужны — ' +
    'чистый дизайн с заголовками и короткими блоками.',
].join(' ');

/** Просит модель сверстать HTML-выжимку по готовому тексту исследования. */
export async function buildDeepDigestHtml(question: string, research: string, signal?: AbortSignal): Promise<string> {
  // Те же умные уровни, что у «!файл html»: OpenRouter-голова, потом Gemini-хвост.
  // Пусто — собирать выжимку некому (само размышление при этом уже готово).
  const levels = resolveSmartLevels(resolveChain(THINK_CHAIN).models, config.files.maxOutputTokens);
  if (levels.length === 0) throw new Error('Нейросеть не подключена — собирать выжимку некому.');

  const answer = await generateWithFallback(
    levels,
    `Вопрос, на который отвечало исследование: ${question}\n\nТекст исследования:\n\n${research}`,
    { systemPrompt: buildInstruction(DIGEST_HTML_HINT), temperature: 0.6, ...(signal ? { signal } : {}) },
  );

  const content = stripFence(answer.text);
  if (!/^</.test(content)) throw new Error('Выжимка приехала не HTML.');
  return content;
}

/** Отправляет готовую HTML-выжимку отдельным файлом. */
export async function sendHtmlDigest(ctx: BotContext, html: string, question: string): Promise<void> {
  const caption =
    `📊 ${escapeHtml(question.slice(0, 200))}\n` + '<i>Наглядная выжимка главного — для телефона.</i>';

  await sendFile(ctx, html, makeFileName(question, 'html'), caption);
}
