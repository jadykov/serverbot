...
/**
 * Десятое слово-переключатель: «/гем !deepseek ...» — один вопрос, много
 * думанья.
 *
 ...
 *
 * Слово названо по модели, а модель — переменная в .env (DEEP_MODEL).
 * Поменяется модель — команда останется этой: её знают наизусть, а имя
 * команды дороже точности названия.
 */
const DEEP_PREFIX = switchWord('deepseek');
 ...
 */
const FORGOTTEN_BANG = /^(?:нарисуй|контекст|скажи|найди|найти|расшифруй|трек|песня|сеть|погугли|deepseek)(?:[\s,:.—–-]|$)/i;
  ...
  if (!rawPrompt) {
    await ctx.reply(
      'Напишите запрос после команды. Например:\n' +
        ...
        '<code>/гем !deepseek что такое понимание</code> — долгое размышление\n\n' +
...
/**
 * Глубокое размышление: «/гем !deepseek ...».
 *
  ...
  if (!question) {
    await ctx.reply(
      'Напишите вопрос, над которым подумать:\n' +
        '<code>/гем !deepseek что значит «понимать» текст</code>\n' +
        '<code>/гем !deepseek стоит ли доверять интуиции в спорах</code>\n\n' +
