 ...
 *   /гем !файл <формат> <что сделать> — ответ отдельным файлом;
  ...
  *   /гем !ответь (реплаем) — то же голосовое, но ответить по существу, без выписки;
 ...
 * Что выпало случайно, то и висит до ответа — текст не меняется.
 * Без смайликов, коротко (решение владельца 2026-09-09).
 */
...
/**
 * Слово-переключатель: «/гем !ответь» реплаем на голосовое.
 *
 * То же, что старое «!расшифруй с ответом», только отдельным словом:
 * содержательный ответ по записи без дословной выписки. Старая форма
 * оставлена алиасом (см. WITH_ANSWER ниже) — привычка не ломается.
 */
const ANSWER_PREFIX = switchWord('ответь|ответить');
 ...
 */
const FORGOTTEN_BANG = /^(?:нарисуй|скажи|найди|найти|расшифруй|послушай|ответь|ответить|трек|песня|сеть|интернет|гугл|погугли|размышление|рассуждение|файл|личка|лс|лимиты)(?:[\s,:.—–-]|$)/i;
 ...
 *
 * За таким разбором идут не за репликой, а за разбором. Файлов у обычных
 * ответов нет (файлом приезжают только явный «!файл» и «!размышление»),
 * поэтому разбор тоже обязан уместиться в одно сообщение — см. CHAT_LENGTH_RULE.
 */
const THINK_RULE = [
  ...
  'Уложись при этом в одно сообщение Telegram (см. правило длины): сокращай, а не раздувай.',
  'Но объём ради объёма не нужен — воды, повторов и пересказа вопроса быть не должно.',
].join(' ');

/**
 * Правило длины обычного ответа в чат: строго одно сообщение Telegram.
 *
 * У Telegram потолок 4096 знаков, отправляем мы по MESSAGE_LIMIT (3500) —
 * это тот же потолок минус ~15% запаса на разворот разметки в HTML-теги.
 * Файлов у обычного ответа нет: файлом приезжают только явный «!файл»
 * и «!размышление», всё остальное обязано уместиться в одно сообщение.
 */
...
/**
 * Старое «!расшифруй с ответом» / «ответом» / «ответь» / «отвечай» — просят
 ...
 * на голосовое ниже). Оставлено алиасом: новым именем стало отдельное
 * «!ответь» (см. ANSWER_PREFIX), привычка не ломается.
 */
...
/**
 * Отдаёт готовый ответ в чат. Файлов здесь нет: файлом приезжают только
 * явный «!файл» (см. handleFile в ./file.ts) и «!размышление» (см. handleDeep
 * ниже) — всё остальное обязано уместиться в одно сообщение Telegram.
 *
 * Страховка на случай, если модель в лимит не уложилась: режем по границе
 * абзаца или предложения тем же приёмом, что и «!сеть» (см. trimToLimit).
...
interface AskOptions {
   ...
   * разбора. Этим путём идут только явные заказы файла (см. handleFile):
   * переросшие сообщение обычные ответы файлом больше не уезжают.
   */
  asFile?: AnswerFormat;
  /**
   ...
   *
   * Обычный ответ обязан уместиться в одно сообщение (см. CHAT_LENGTH_RULE
...
/**
 * `byNeed` — обычный вопрос в чат: модель просят уложиться в одно сообщение
 * Telegram (см. CHAT_LENGTH_RULE), токенные потолки уровней остаются.
 *
...
async function askChain(
   ...
   * обычный обязан уместиться в одно сообщение (см. CHAT_LENGTH_RULE).
   * Потолки по токенам при этом остаются всегда: знаки модель не считает,
   * и обещаниям про длину верить нельзя — что не влезло, режет sendAnswer.
   *
  ...
  const rules = [
    ...
    length === 'byNeed' ? CHAT_LENGTH_RULE : undefined,
  ...
  try {
    ...
    // Текст статичен: что выпало случайно, то и висит до ответа.
    if (thinkingPlaceholder) placeholder = await sendThinkingPlaceholder(ctx);
...
async function handleGemini(ctx: BotContext, rawPrompt: string): Promise<void> {
  ...
  const answerWord = ANSWER_PREFIX.exec(trimmed);
  const ownsVoice =
    repliedVoice &&
  ...
  if (repliedVoice && ownsVoice) {
    // «!ответь» — ответ по существу без выписки; остаток после слова (если есть) —
    // конкретный вопрос по записи. «!расшифруй выпиши дословно» — слово съедаем,
    // остаток остаётся вопросом.
    const answerAsked = answerWord ? (answerWord[1] ?? '').trim() : undefined;
    const asked = answerAsked ?? (voiceWord ? (voiceWord[1] ?? '').trim() : trimmed);
    // Голое «!расшифруй» и явные синонимы (TO_TEXT) — дословная выписка,
    // это теперь умолчание. «!ответь» и старое «!расшифруй с ответом» —
    // содержательный ответ. Любой другой текст после команды — конкретный
    // вопрос по записи, как и раньше.
    const task =
      answerAsked !== undefined
        ? answerAsked || VOICE_DEFAULT_TASK
        : WITH_ANSWER.test(asked)
          ? VOICE_DEFAULT_TASK
 ...
 * «агентский»: жирный — только заголовок и названия блоков, подчёркивания
...
function newsFinalRule(charBudget: number): string {
  ...
  return [
    ...
    '3) затем строка **Главное** и 3–6 коротких пунктов, каждый с новой строки начинается с символа • — по одному факту в пункте, в конце каждого пункта точка;',
    ...
    'Каждое предложение заканчивай точкой (или ? ! …), лид и пункты списка — тоже с точкой в конце; главный заголовок в первой строке — без точки в конце. Каждое новое предложение начинай с заглавной буквы.',
