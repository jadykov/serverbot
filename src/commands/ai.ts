 ...
 * поломкой.
 *
 * charBudget — сколько знаков реально осталось под сам текст после того,
 * как вычли место под список источников (см. sendWebAnswer). Не подсказка
 * «примерно», а число, под которое дальше жёстко обрежет sendWebAnswer,
 * если модель всё же промахнётся мимо него.
 */
function newsFinalRule(charBudget: number): string {
  ...
  return [
    ...
    'Начни с прямого ответа на вопрос в двух-трёх строках, дальше — подробности короткими',
    'абзацами или списком: сплошной простынёй такой объём не читается.',
 ...
 *
 * Место под ссылки вычитается из потолка заранее: они идут последними,
 * и при обрезке по границе сообщения пропали бы первыми, — а ради них поиск
 * и затевался. Сам текст к этому моменту уже нацелен моделью в нужный
 * диапазон длины (см. newsFinalRule) — обрезка здесь только страховка
 * на случай, если модель попросила больше, чем ей разрешили.
 */
async function sendWebAnswer(ctx: BotContext, text: string, links: string[]): Promise<void> {
  const sources = sourcesBlock(links);
  const budget = Math.floor(MESSAGE_LIMIT * NEWS_LENGTH_MAX_RATIO) - sources.length;

  await sendMarkdown(ctx, trimToLimit(text, Math.max(budget, 0)) + sources);
}
