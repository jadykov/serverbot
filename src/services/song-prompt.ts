...
function instruction(): string {
  ...
  return [
    ...
    '',
    'Соразмеряй слова с выбранной длительностью: примерно одна короткая строка',
    `на четыре секунды, то есть для ${duration} секунд — около ${Math.max(2, Math.round(duration / 4))} строк.`,
