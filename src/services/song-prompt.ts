 ...
 * Поэтому черновик собирает бесплатный Gemini на быстрой формальной цепочке
 * (второй уровень — GEMINI_CHAIN_FAST, тот же, что у свободного разбора «!скажи», см.
 * planSpeech в services/gemini-tts.ts): задача формальная, и тяжёлая модель
 * тратит на неё двадцать секунд, ничего не выигрывая. Рисование пользуется
 * той же быстрой цепочкой (см. planner в commands/draw.ts).
 *
 ...
 * на середине фразы чаще, чем оставался с запасом. Фиксированные 6-8 строк
