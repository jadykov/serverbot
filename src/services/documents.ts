 ...
 * и выше — уже нет.
 *
 * Отсюда предел по умолчанию: 0 — без ограничения (решение владельца).
 * ВНИМАНИЕ: без предела минутная норма Gemini (16 000 токенов/мин
 * на бесплатном тарифе) съедается одним толстым файлом целиком, и модель
 * на минуту замолкает для всей группы. На бесплатном ключе разумно держать
 * DOC_MAX_TOKENS=150000 (около 450 КБ текста или 230 страниц).
 *
...
export async function prepareDocument(data: Buffer, fileName: string): Promise<PreparedDocument> {
  ...
  // 0 — без ограничения (см. DOC_MAX_TOKENS): тогда и проверять нечего.
