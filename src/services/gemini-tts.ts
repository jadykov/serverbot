 ...
 */
function speechPlanRules(hasContext: boolean): string {
  return [
    ...
    ...(hasContext
      ? [
          'Важный случай: просьба может целиком описывать только голос и манеру —',
          '«голосом старого деда», «голосом продавца шаурмы», «голосом бандита» — без единого',
          'слова о том, что, собственно, произнести. Тогда весь текст просьбы — это "direction",',
          'а "text" бери из сообщения, приложенного отдельно ниже как контекст (реплай в чате):',
          'именно его и нужно озвучить голосом, который описали. Не путай направление с текстом —',
          '«голосом бандита» не читается вслух само по себе, это не то, что нужно произнести.',
          '',
        ]
      : []),
 ...
 *
 * context — реплай на сообщение («!скажи голосом старого деда» ответом
 * на чужую реплику): без него запрос вроде «голосом бандита» — это только
 * указание, что сказать вообще нечего, и планировщик раньше молчал. Теперь
...
export async function planSpeech(
  ...
  const rules = speechPlanRules(Boolean(context));
  const userMessage = context ? `${request}\n\nКонтекст (реплай в чате):\n${context}` : request;
  // Если разбор совсем не удастся, разумный запасной текст — контекст,
  // а не сама просьба: «голосом бандита» озвучивать буквально нечего.
  ...
  for (const attempt of [1, 2] as const) {
    ...
    const plan = toSpeechPlan(answer.text, fallback);
    if (plan) return plan;

    logger.warn('Помощник озвучки вернул не JSON, пробую ещё раз', { attempt });
  }

  return { text: fallback, direction: '' };
}
