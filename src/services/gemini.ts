...
/**
 * Собирает системную инструкцию: роль своя или общая, а правила оформления
 * дописываются всегда.
 *
 * Раньше промпт раздела подставлялся вместо всей инструкции целиком, и вместе
...
function buildSystemPrompt(custom?: string): string {
  return [custom?.trim() || DEFAULT_ROLE, MARKUP_RULES, EMOJI_RULES].join(' ');
}
