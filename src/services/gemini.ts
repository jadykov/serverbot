...
const MARKUP_RULES = [
  ...
  '**жирный** — только для заголовков и названий разделов, внутри текста жирным ничего не выделяй;',
  '++подчёркнутый++ — только при реальной необходимости (без выделения смысл теряется): максимум 1–2 слова на ответ, чаще — вообще без подчёркиваний. По умолчанию не подчёркивай ничего;',
  '`инлайн-код` — только там, где текст реально нужно скопировать тапом (команда, код, значение для вставки); термины, названия и важное кодом не выделяй, для них есть жирный и курсив. Блоки кода — в тройных обратных кавычках с указанием языка,',
  'ссылки вида [текст](https://...), цитаты через >, списки на •.',
  '*курсив* — редко: только для названий произведений и чужих терминов, не для выделения важного.',
  'Не используй таблицы, вложенные списки и HTML-теги.',
  'Каждый заголовок — обязательно **жирным**, заголовок plain-текстом без ** запрещён. Заголовки допустимы, но не глубже ###.',
  'Дыши абзацами: каждый смысловой блок — отдельным абзацем с пустой строкой между ними; заголовок отделяй пустой строкой сверху и снизу; список — пустыми строками до и после; сплошная стена текста без пустых строк запрещена. Внутри абзаца — 2–4 предложения, не больше.',
...
/**
 * Смайлики — акцент и строгая разметка, а не украшение.
 *
 * Решение владельца: полный запрет смотрелся сухо, поэтому разрешён минимум —
 * либо 1 цветной по настроению сообщения, либо 1 цветной + 1 текстовый
 * (· →), либо 2 текстовых. Структурные маркеры списков (•) под лимит
 * не попадают. В длинном ответе сверху идут строгие маркеры
 * (📌 ❗️ ⚠️ ✅ в начале заголовка или важной строки, текстовые • → ·
 * для структуры) — как разметка для чтения с телефона.
 * Уместность обязательна: не в каждое сообщение, в коротком
 * ответе на две-три строки — ничего.
 */
const EMOJI_RULES = [
  'Смайлики — акцент, а не украшение: на сообщение либо 1 цветной по его настроению,',
  'либо 1 цветной + 1 текстовый (· →), либо 2 текстовых. Больше двух значков настроения нельзя, цветных — максимум один.',
  'В длинном ответе (от 4–5 абзацев) обязательно освежай текст строгими маркерами-разметкой, чтобы легко читалось с телефона:',
  'каждый заголовок может начинаться с одного маркера (📌 ❗️ ⚠️ ✅), важные строки — тоже;',
  'текстовые • → · для структуры — свободно; ориентир — маркер в каждом втором-третьем блоке,',
  'но не чаще одного маркера в строке, не в каждую строку подряд.',
  'Ставь только уместно и не в каждое сообщение: в коротком ответе на две-три строки смайликов и маркеров нет совсем.',
  'Не в середине слов и предложений, не в коде, не подряд.',
].join(' ');
...
/**
 * Запрет отвечать самоотчётом о соблюдении инструкций вместо самого ответа.
 *
 * Баг найден вживую 23.08.2026 на «/гем !сеть» с gemini-3.5-flash: вместо
 * ответа про новости бот показал построчный разбор своих же правил —
 * «"В коротком ответе..." -> This is a detailed response... so emojis are
 * allowed» и так далее для каждого пункта. Модель не нарушила ни одно
 * правило разметки — она вместо ответа написала на английском отчёт о том,
 * что все правила выполнены. Инструкция и так длинная (роль + разметка +
 * смайлики + запрет инструментов + правила поиска), и слабая быстрая модель
 * иногда принимает список требований за то, что от неё просят подтвердить,
 * а не выполнить молча.
 */
const NO_META_RULES = [
  'Никогда не пиши отчёт или разбор того, как ты выполнил инструкции из этого',
  'системного промпта: ни построчную сверку правил, ни фразы вида «правило -> выполнено»,',
  'ни оговорки о том, что ты сейчас следуешь заданному формату. Сразу пиши сам ответ',
  'для человека, без единого слова о самих инструкциях.',
].join(' ');
...
function buildSystemPrompt(custom?: string, extra?: string): string {
  return [custom?.trim() || DEFAULT_ROLE, MARKUP_RULES, EMOJI_RULES, NO_TOOLS_RULES, NO_META_RULES, extra?.trim() ?? '']
    .join(' ')
    .trim();
}
