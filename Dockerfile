...
# ffmpeg нужен для звука в обе стороны, и оба раза — ради формата:
#   • озвучка: Gemini отдаёт сырой PCM, а Telegram принимает голосовые
#     исключительно в OGG/Opus;
#   • голосовые на вход: Telegram отдаёт OGG/Opus, а Google документирует
#     OGG Vorbis, и запись надёжнее перекодировать в FLAC.
# Без ffmpeg бот работает ровно так же, только хуже по краям: озвучка уедет
# обычным аудиофайлом (WAV) — играется не хуже, но весит вдесятеро больше
# и выглядит как вложение, — а голосовое уйдёт в модель как есть, и примет
# ли Google Opus, выяснится на месте.
